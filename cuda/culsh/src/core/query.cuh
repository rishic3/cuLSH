...
static __device__ int find_bucket_in_table(const uint8_t* __restrict__ all_bucket_signatures,
    ...
    while (lo <= hi) {
        int mid = lo + (hi - lo) / 2;
        const uint8_t* mid_sig = all_bucket_signatures + static_cast<size_t>(mid) * sig_nbytes;

        // Check if signature matches
        int cmp = 0;
        for (int i = 0; i < sig_nbytes && cmp == 0; ++i) {
...
static __global__ void find_matching_buckets_kernel(
    ...
    const uint8_t* query_sig =
        Q_sig + static_cast<size_t>(table_idx) * (static_cast<size_t>(n_queries) * sig_nbytes) +
        static_cast<size_t>(query_idx) * sig_nbytes;
    ...
    int matched_bucket =
        find_bucket_in_table(all_bucket_signatures, table_start, table_end, query_sig, sig_nbytes);
...
/**
 * @brief Query LSH index to find candidates from matched bucket IDs
 * @param[in] stream CUDA stream
 * @param[in] d_matched_bucket_indices Device pointer of shape [n_queries * n_hash_tables] laid out
 *            pair-major: i.e., d_matched_bucket_indices[q * n_hash_tables + t] = bucket_id (or -1)
 * @param[in] n_queries Number of queries
 * @param[in] index Device index
 * @return Candidates object
 */
inline Candidates query_from_matched_buckets(cudaStream_t stream,
    ...
    // Temp storage reused across CUB calls
    ...
    // Compute total raw candidates on host - this includes all candidates
    // collected per query across all tables, including duplicates
    size_t total_candidates_offset = 0;
    size_t last_query_count = 0;
    ...
    CUDA_CHECK(cudaMemcpyAsync(&last_query_count,
                               ...
                               cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream)); // sync to ensure copies to host are complete
    ...
    // Cleanup temp storage used during collection
    CUDA_CHECK(cudaFree(d_candidate_counts));
    CUDA_CHECK(cudaFree(d_table_prefix_offsets));
    ...
    cub::DeviceSegmentedRadixSort::SortKeys(
        ...
        n_queries, candidates.query_candidate_offsets, candidates.query_candidate_offsets + 1, 0,
        sizeof(int) * 8, stream);
    ...
    // New candidate counts after de-dup (segmented sum on flags)
    ...
    cub::DeviceSegmentedReduce::Sum(
        d_temp_storage, temp_storage_bytes, d_flags, candidates.query_candidate_counts, n_queries,
        candidates.query_candidate_offsets, candidates.query_candidate_offsets + 1, stream);
    ...
    // Store total unique candidates on host
    CUDA_CHECK(cudaMemcpyAsync(&total_candidates_offset,
                               candidates.query_candidate_offsets + (n_queries - 1), sizeof(size_t),
                               cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaMemcpyAsync(&last_query_count,
                               candidates.query_candidate_counts + (n_queries - 1), sizeof(size_t),
                               cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream)); // sync to ensure copies to host complete
 ...
 * @return Candidates object
 */
inline Candidates query_index(cudaStream_t stream, const void* Q_sig, int n_queries, int sig_nbytes,
                              const Index* index) {
    assert(sig_nbytes > 0 && "sig_nbytes must be > 0");
    assert(index != nullptr && "index must be non-null");
    assert(index->sig_nbytes == sig_nbytes && "sig_nbytes != index->sig_nbytes");
    const uint8_t* Q_sig_bytes = static_cast<const uint8_t*>(Q_sig);
    ...
    find_matching_buckets_kernel<<<grid_size_items, block_size, 0, stream>>>(
        Q_sig_bytes, index->all_bucket_signatures, index->table_bucket_offsets, n_queries,
        n_hash_tables, sig_nbytes, d_matched_bucket_indices);

    Candidates candidates =
        query_from_matched_buckets(stream, d_matched_bucket_indices, n_queries, index);
