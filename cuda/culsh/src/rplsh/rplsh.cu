...
RPLSHIndex fit(cublasHandle_t cublas_handle, cudaStream_t stream, const float* X, int n_samples,
    ...
    // allocate X_hash
    float* X_hash;
    CUDA_CHECK(cudaMalloc(&X_hash, static_cast<size_t>(n_samples) * params.n_hash_tables * params.n_projections * sizeof(float)));
    ...
    int8_t* X_sig;
    CUDA_CHECK(cudaMalloc(&X_sig, static_cast<size_t>(n_samples) * params.n_hash_tables * params.n_projections * sizeof(int8_t)));
    detail::compute_signatures<float>(stream, X_hash, n_samples, params.n_hash_tables, params.n_projections, X_sig);
    CUDA_CHECK(cudaFree(X_hash));  // done with X_hash

    // build and return index
    auto index = detail::build_index(stream, X_sig, n_samples, params.n_hash_tables, params.n_projections);
...
RPLSHIndex fit(cublasHandle_t cublas_handle, cudaStream_t stream, const double* X, int n_samples,
    ...
    // allocate X_hash
    double* X_hash;
    CUDA_CHECK(cudaMalloc(&X_hash, static_cast<size_t>(n_samples) * params.n_hash_tables * params.n_projections * sizeof(double)));
    ...
    int8_t* X_sig;
    CUDA_CHECK(cudaMalloc(&X_sig, static_cast<size_t>(n_samples) * params.n_hash_tables * params.n_projections * sizeof(int8_t)));
    detail::compute_signatures<double>(stream, X_hash, n_samples, params.n_hash_tables, params.n_projections, X_sig);
    CUDA_CHECK(cudaFree(X_hash));  // done with X_hash
