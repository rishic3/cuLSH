...
__global__ void extract_byte_key_kernel(const int8_t* X_sig, const uint32_t* item_indices,
                                        int n_rows, int n_hash_tables, int n_projections,
    ...
    uint32_t table_id = original_item_idx / n_rows;
    uint32_t row_id = original_item_idx % n_rows;

    const int8_t* sig_ptr =
        X_sig + table_id * (static_cast<size_t>(n_rows) * n_projections) + row_id * n_projections;

    // d_keys = single byte per item
 ...
 */
__global__ void extract_table_id_key_kernel(const uint32_t* item_indices, int n_rows,
                                            size_t n_items, uint8_t* d_keys) {
    size_t idx = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (idx >= n_items)
        return;

    // integer divide to get table id
    uint32_t original_item_idx = item_indices[idx];
    d_keys[idx] = static_cast<uint8_t>(original_item_idx / n_rows);
}
...
__global__ void mark_boundaries_kernel(const int8_t* X_sig, const uint32_t* sorted_item_indices,
    ...
    uint32_t table_id_curr = orig_idx_curr / n_rows;
    ...
    uint32_t table_id_prev = orig_idx_prev / n_rows;
    ...
    if (d_table_flags[idx] == 1) {
        ...
        uint32_t row_id_curr = orig_idx_curr % n_rows;
        uint32_t row_id_prev = orig_idx_prev % n_rows;
        const int8_t* sig_ptr_curr = X_sig +
                                     table_id_curr * (static_cast<size_t>(n_rows) * n_projections) +
                                     row_id_curr * n_projections;
        const int8_t* sig_ptr_prev = X_sig +
                                     table_id_prev * (static_cast<size_t>(n_rows) * n_projections) +
...
__global__ void build_final_index_kernel(const int8_t* X_sig, const uint32_t* sorted_item_indices,
    ...
    uint32_t orig_row_id = orig_idx % n_rows;

    // store original row idx in all_candidate_indices in sorted order
    d_all_candidates[idx] = orig_row_id;

    if (d_bucket_flags[idx] == 1) {
        // this item is the first of a new bucket
        // write sorted position to bucket_candidate_offsets
        int bucket_idx = d_bucket_scan[idx];
        d_bucket_candidate_offsets[bucket_idx] = idx;

        // copy signature for this bucket to d_bucket_signatures
        uint32_t table_id = orig_idx / n_rows;
        const int8_t* sig_src_ptr = X_sig +
                                    table_id * (static_cast<size_t>(n_rows) * n_projections) +
...
RPLSHIndex build_index(cudaStream_t stream, const int8_t* X_sig, int n_rows, int n_hash_tables, int n_projections) {
    ...
    // sort item indices lexicographically, starting from least significant to most significant byte of signature
    for (int byte_idx = n_projections - 1; byte_idx >= 0; --byte_idx) {
        ...
        extract_byte_key_kernel<<<grid_size, block_size, 0, stream>>>(
            X_sig, d_item_indices, n_rows, n_hash_tables, n_projections, byte_idx, d_keys);
        cub::DeviceRadixSort::SortPairs(d_temp_storage, temp_storage_bytes, d_keys, d_temp_keys,
                                        d_item_indices, d_temp_indices, n_items, 0, 8, stream);
    ...
    //                  ...]
    extract_table_id_key_kernel<<<grid_size, block_size, 0, stream>>>(d_item_indices, n_rows,
                                                                      n_items, d_keys);
    cub::DeviceRadixSort::SortPairs(d_temp_storage, temp_storage_bytes, d_keys, d_temp_keys,
                                    d_item_indices, d_temp_indices, n_items, 0, 8, stream);
    std::swap(d_item_indices, d_temp_indices);
    ...
    mark_boundaries_kernel<<<grid_size, block_size, 0, stream>>>(
        X_sig, d_item_indices, n_rows, n_hash_tables, n_projections, n_items, d_bucket_flags,
        d_table_flags);
    ...
    // run exclusive sum to get bucket offsets from binary flags
    ...
    build_final_index_kernel<<<grid_size, block_size, 0, stream>>>(
        X_sig, d_item_indices, d_bucket_flags, d_bucket_scan, n_rows, n_hash_tables,
