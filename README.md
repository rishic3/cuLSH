...
> **Note:** cuLSH uses bucket-based retrieval (returns all bucket matches), while FAISS IndexLSH uses hash-then-scan (computes Hamming distance to all vectors and returns top-k). FAISS k param was set to match mean candidates returned by cuLSH for a close comparison. Comparison of recall, however, doesn't exactly sense here given the algorithmic differences.
