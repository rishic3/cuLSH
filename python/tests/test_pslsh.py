...
import numpy as np
import pytest
from ref_lsh import PStableLSH as RefPSLSH
from utils import evaluate_recall_at_k, generate_dense_data

from culsh import PStableLSH, PStableLSHModel
from culsh.utils import compute_recall


def get_euclidean_top_k(
    X: np.ndarray,
    Q: np.ndarray,
    query_idx: int,
    k: int,
) -> np.ndarray:
    """Get top-k indices from X by Euclidean distance to Q[query_idx]"""
    q = Q[query_idx]
...
def test_pslsh_recall_vs_reference(
    dtype, n_samples, n_hash_tables, n_hashes, window_size
):
    """Test cuLSH p-stable LSH against reference CPU implementation."""
    THRESHOLD = 0.05
    ...
    for q_idx in range(n_eval):
        gt = get_euclidean_top_k(X, X, q_idx, k)
    ...
    culsh_recalls = evaluate_recall_at_k(
        X, X, indices, offsets, get_top_k_fn=get_euclidean_top_k, k=k  # type: ignore
    )
...
def test_pslsh_save_load(cupy):
    """Test p-stable LSH save and load."""
    THRESHOLD = 0.00001
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        model.save(os.path.join(tempdir, "test_pslsh.npz"))
        loaded_model = PStableLSHModel.load(os.path.join(tempdir, "test_pslsh.npz"))
    ...
    # Check recall
    def query_and_get_recall(model: PStableLSHModel, X, Q) -> float:
        ...
        recalls = evaluate_recall_at_k(
            ...
            get_top_k_fn=get_euclidean_top_k,  # type: ignore
            k=k,
        )
...
@pytest.mark.parametrize("cupy", [False, True])
def test_pslsh_fit_query_consistency(cupy):
...
def test_pslsh_batched_query_consistency(cupy, batch_size):
